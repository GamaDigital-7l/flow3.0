...
const HealthGoalForm: React.FC<HealthGoalFormProps> = ({ initialData, onGoalSaved, onClose }) => {
  ...
  const saveGoalMutation = useMutation({
    ...
    onSuccess: () => {
      showSuccess(`Meta de saúde ${initialData?.id ? 'atualizada' : 'adicionada'} com sucesso!`);
      queryClient.invalidateQueries({ queryKey: ["healthGoals", userId] });
      onGoalSaved();
    },
    onError: (error: any) => {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 p-4 bg-card rounded-xl card-hover-effect">
        <div>
          ...
          <Input id="title" {...form.register("title")} placeholder="Ex: Perder 5kg" />
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Input id="unit" {...form.register("unit")} placeholder="Ex: kg, km, dias" />
