...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Receber o ID do evento do Google Calendar para deletar
    const { googleEventId } = await req.json();

    if (!googleEventId) {
      return new Response(
        JSON.stringify({ error: "Missing googleEventId for deletion." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // 4. Deletar o evento do Google Calendar
    ...
    if (!googleCalendarResponse.ok) {
      // Se o evento já não existe no Google Calendar (status 404), ainda consideramos sucesso
      if (googleCalendarResponse.status === 404) {
        console.warn(`Evento Google Calendar ${googleEventId} não encontrado, mas a exclusão foi solicitada. Prosseguindo.`);
      } else {
    ...
    return new Response(JSON.stringify({ message: "Google Calendar event deleted successfully." }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });

  } catch (error) {
    console.error("Erro na Edge Function delete-google-calendar-event:", error);
