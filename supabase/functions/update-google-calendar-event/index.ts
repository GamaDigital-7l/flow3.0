...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Receber os dados do evento atualizados do corpo da requisição
    const { googleEventId, title, description, date, startTime, endTime, location } = await req.json();

    if (!googleEventId || !title || !date || !startTime) {
      return new Response(
        JSON.stringify({ error: "Missing required event fields: googleEventId, title, date, or startTime." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
    ...
    if (!googleCalendarResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to update Google Calendar event.", details: errorData }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    const updatedEvent = await googleCalendarResponse.json();

    return new Response(JSON.stringify({ googleEventId: updatedEvent.id, htmlLink: updatedEvent.htmlLink }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });

  } catch (error) {
    console.error("Erro na Edge Function update-google-calendar-event:", error);
